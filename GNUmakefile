...
test\:integration\:zia:
	...
	@TF_ACC=1 go test -v -race -cover -coverprofile=coverage.out -covermode=atomic ./zia -parallel 10 -timeout 120m
...
test\:integration\:zscalerone:
	...
	@TF_ACC=1 go test -v -race -cover -coverprofile=coverage.out -covermode=atomic ./zia -parallel 10 -timeout 120m -run ^$(integration_tests)$$
	go tool cover -html=coverage.out -o coverage.html
...
test\:integration\:zscalertwo:
	...
	@TF_ACC=1 go test -v -race -cover -coverprofile=coverage.out -covermode=atomic ./zia -parallel 10 -timeout 120m -run ^$(integration_zs2_tests)$$
	go tool cover -html=coverage.out -o coverage.html
