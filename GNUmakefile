...
build13: DESTINATION=$(APPDATA)/terraform.d/plugins/$(ZIA_PROVIDER_NAMESPACE)/1.0.1/$(GOOS)_$(GOARCH)
else
build13: DESTINATION=$(HOME)/.terraform.d/plugins/$(ZIA_PROVIDER_NAMESPACE)/1.0.1/$(GOOS)_$(GOARCH)
endif
build13: fmtcheck
	go mod vendor
	go mod tidy
	...
	@mkdir -p $(DESTINATION)
	go build -o $(DESTINATION)/terraform-provider-zia_v1.0.1
